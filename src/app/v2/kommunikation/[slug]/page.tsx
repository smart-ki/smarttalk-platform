...
export default async function V2KommKursDetailPage({ params }: Props) {
  const kurs = await getKommKursBySlug(params.slug)
  if (!kurs) notFound()
    ...
    <>
      ...
      <div className="bg-cream-200 py-3 px-4">
        <div className="max-w-5xl mx-auto text-sm text-ink-muted flex items-center gap-2">
          <Link href="/v2" className="hover:text-mauve-600 transition-colors">Startseite</Link>
          <span>/</span>
          <Link href="/v2#kommunikation" className="hover:text-mauve-600 transition-colors">Kommunikation</Link>
          <span>/</span>
        ...
        <section className="bg-cream-200 py-16 px-4">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              {andere.map((k) => (
                <Link key={k.slug} href={`/v2/kommunikation/${k.slug}`} className="bg-white rounded-3xl shadow-soft p-6 hover:shadow-soft-lg transition-shadow group">
            ...
            <div className="mt-8">
              <Link href="/v2#kommunikation" className="btn-v2-mauve text-sm py-2.5 px-5">Alle Kommunikations-Kurse anzeigen →</Link>
            </div>
          </div>
        </section>
      )}
